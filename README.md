...
**Included:** Six stories covering products, policy, research, and education; dates, sources, reading times, summaries, significance explanations, original-source links, a responsive layout, category filters, and bookmarks saved in the current browser.
...
1. All six stories appear with working original-source links and retain their source information, summaries, and significance explanations.
2. The page remains readable and usable at a 1440-pixel desktop width and a 390-pixel narrow-screen width.
3. `npm run build` succeeds, creates `dist/index.html`, and that generated file contains the six story titles.
...
| --- | --- | --- |
| `src/pages/index.astro` | Home-page route, page template, filters, bookmarks, and mobile-menu script | Edited |
...
| `src/data/stories.ts` | Typed content records for all six stories | Edited |
...
This excerpt from `src/pages/index.astro` shows the boundary between build-time work and template output:

```astro
---
// Frontmatter: Astro evaluates this while generating the page.
import StoryCard from '../components/StoryCard.astro'; // Import
import { stories } from '../data/stories';             // Import

const categories = ['All', 'Companies & products', 'Policy', 'Research', 'Education']; // Declaration
---

<!-- Template markup and an Astro expression -->
...
1. `src/data/stories.ts` stores the title `OpenAI shares first results from its custom Jalapeño inference chip` in a story record.
2. `index.astro` imports the `stories` array.
3. `stories.map(...)` visits that record and passes the complete `story` object into `StoryCard` as a prop.
...
Frontmatter cannot respond to a reader's click because it runs before the static page reaches the browser. The normal `<script>` in `index.astro` runs in the browser, where `document`, click events, and `localStorage` exist. That script handles filtering, bookmarks, and mobile navigation.
...
The following commands completed successfully on September 6, 2026:
...
Astro reported one generated page, completed the production build, and served the production preview at `http://127.0.0.1:4323/`. The generated `dist/index.html` contains the page heading and all six story titles.
